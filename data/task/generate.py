...
class Generate:
    def __init__(self, number_of_online_user = 10):
        self.no_user_interval = number_of_online_user
    ...
    def generate_task_info(self):
        for i in range(self.number_of_time_slot):
            with open("{}/datatask{}.csv".format(str("timeslot_datasets"),i),"w") as output:
                ...
                no_user = rd.randint(self.no_user_interval[0], self.no_user_interval[1])
                ...
                for point in range(number_of_point_per_time_slot):
                    ...
                    indexs = np.sort(np.random.randint(i * 300, (i + 1) * 300), no_task_per_point)
                    points = points + indexs

                n = len(points)
                output.write("time step,user,task data size,task computing size,time threshold")
                for j in range(n):
